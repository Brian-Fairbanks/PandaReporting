...
def checkFile(df):
    ...
    c0 = fc.check0(df)
    if c0 is not None:
        df = getCellFix(df, c0, ["Earliest Time Phone Pickup AFD or EMS"], curError)
    else:
        print("passed")
    ...
    c1 = fc.check1(df)
    if c1 is not None:
        getCellFix(df, c1, ["FirstArrived"], curError)
    else:
        print("passed")
    ...
    c2 = fc.check2(df)
    if c2 is not None:
        getCellFix(df, c2, ["Unit Time Arrived At Scene"], curError)
    else:
        print("passed")
