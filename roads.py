...
def distToStationFromNode(dest_node, fullProgress=None, progress=None):
    """
...
def getRoads():
    ...
    if not exists("./data/roads/roadsProjected.graphml"):
        ...
        try:
            np.array(GCon.nodes).astype(np.int64)
        except ValueError:
            GCon = nx.convert_node_labels_to_integers(GCon)
        # GProj = ox.project_graph(G)
        # GCon = ox.consolidate_intersections(
        #     GProj, rebuild_graph=True, tolerance=1, dead_ends=False
        # )
        # GCon = rebuild_graph
